...
# Description: Get weibo_id, weibo_name and account_id from MySOL database,
...
# def clear_redis(): # dangerous function
# 	"""
...
def output_from_mysql_to_redis(header_list, data_list):
    ...
    # write all account_id into a redis set and list, and HASHes of every
    # account_id
    for single_info in data_list:
        account_id_tmp = str(single_info[0])

        # add account_id in a set used for weixin.gogou.com
        # remove the account_id which has been crawled
        # if (int(account_id_tmp) in second_found_account_id):
        # 	continue
        ...
        r.hmset(
            hash_name_tmp, {header_list[1]: single_info[1], header_list[2]: single_info[2]})
